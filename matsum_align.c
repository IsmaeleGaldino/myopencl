...
cl_event matinit(cl_command_queue q, cl_kernel k, cl_mem mat, cl_uint nrows, cl_uint ncols, cl_uint pitch, size_t lws_in){
    ...
    err = clSetKernelArg(k,arg++,sizeof(pitch),&pitch);
    ocl_check(err,"matinit clSetKernelArg pitch");
...
cl_event matsum(cl_command_queue q, cl_kernel k, cl_mem mat_a, cl_mem mat_b, cl_mem mat_r, cl_uint nrows, cl_uint ncols, cl_uint pitch, size_t lws_in){
    ...
    err = clSetKernelArg(k,arg++,sizeof(pitch),&pitch);
    ocl_check(err,"matsum clSetKernelArg pitch");
...
void verify(cl_int * mat, cl_uint nrows, cl_uint ncols, cl_uint pitch){
    for(cl_uint r = 0 ; r < nrows ; r++){
        for(cl_uint c = 0 ; c < ncols ; c++){

            if( mat[r*pitch + c] != (2 * c) - (2 * r) ) {
                fprintf(stderr,"(%u,%u) not good \n",r,c);
                exit(2);
            }
        }
    }
...
int main(int argc, char * argv []){
    ...
    if(argc != 4){
        fprintf(stderr,"invalid arguments: ./matsum_align nrows ncols lws\n");
        exit(1);
    }
    ...
    cl_uint bits_align;
    err = clGetDeviceInfo(d,CL_DEVICE_MEM_BASE_ADDR_ALIGN,sizeof(cl_uint),&bits_align,NULL);

    cl_uint pitch = round_mul_up(ncols,(bits_align/8)/sizeof(cl_int));
    const size_t memsize = pitch * nrows * sizeof(cl_int);
    ...
    cl_kernel matinit_k = clCreateKernel(src,"matinit_align",&err);
    ocl_check(err,"matinit clCreateKernel");

    cl_kernel matsum_k = clCreateKernel(src,"matsum_align",&err);
    ocl_check(err,"matsum clCreateKernel");

    cl_event matinit_evt_a = matinit(q,matinit_k,mat_a,nrows,ncols,pitch,lws_in);
    cl_event matinit_evt_b = matinit(q,matinit_k,mat_b,nrows,ncols,pitch,lws_in);

    clFinish(q);

    cl_event matsum_evt = matsum(q,matsum_k,mat_a,mat_b,mat_r,nrows,ncols,pitch,lws_in);
    ...
    verify(result,nrows,ncols,pitch);
